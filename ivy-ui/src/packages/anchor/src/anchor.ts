class Anchor extends HTMLElement {
    ...
    connectedCallback() {
        const scrollContainer: HTMLElement = document.querySelector(this.container) || document.body;
        const scrollContainerStyle = getComputedStyle(scrollContainer);

        if (!["relative", "absolute", "sticky"].includes(scrollContainerStyle.position)) {
        ...
        [...children].forEach(element => {
            ...
            const targetEl: HTMLElement = document.querySelector(href);
            if (targetEl) {
